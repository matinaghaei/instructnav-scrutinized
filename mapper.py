...
class Instruct_Mapper:
    ...
    def get_action_affordance(self,action):
        # try:
        if type(action) is str and 'LLM' in action and (not self.object_entities or not self.object_clusters):
            action = 'Explore'
        if action == 'Explore':
            ...
            return affordance.cpu().numpy()
        elif action == 'Distance-based Frontier':
            ...
            for i, frontier in enumerate(self.frontier_points):
                ...
                affordance[distance <= 0.2] = distance_affordance[distance <= 0.2] * distance_scores[i]
            return affordance
        elif action == 'LLM':
            affordance = np.zeros((self.navigable_pcd.point.positions.shape[0],),dtype=np.float32)
